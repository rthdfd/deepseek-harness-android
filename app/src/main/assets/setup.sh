...
if [ -f "$R/lib/bin.js" ]; then
  printf '%s\n' "DSH found; applying patches only"
else
  printf '%s\n' "DSH missing; installing"
  pkg update -y
  pkg upgrade -y
...
if [ ! -d "$R/node_modules/@img/sharp-wasm32" ]; then
  D="$(mktemp -d)"
  trap 'rm -rf "$D"' EXIT
  printf '{"private":true}\n' >"$D/package.json"
...
B="$P/bin/dsh"
if [ ! -f "$B" ] || ! grep -q -- '--expose-internals' "$B"; then
  rm -f "$B"
